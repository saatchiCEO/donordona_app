...
window.serverError = (data, status, headers, config) ->
  alert "Ошибка связи с сервером: " + status
...
window.DonorDonaApp.config ($stateProvider,   $routeProvider,   $urlRouterProvider, $locationProvider) ->
  $stateProvider
    .state('index',
      url: "/"
      views: 
    ...
    .state('callbacks',
      ...
      onExit: ()-> clearInterval window.refresher
    )
